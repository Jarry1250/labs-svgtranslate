...
require_once( '/home/jarry/public_html/svgtranslate/licence.php' );
require_once( '/home/jarry/public_html/svgtranslate/language.php' );
require_once( '/home/jarry/public_html/i18n.php' );
...
class SVGtranslate{
	 ...
	 * @global array From the Peachy API framework
	...
	private function get_counter(){
		return Counter::get_counter( 'svgtranslate/count_text.txt' );
	}
	...
	public function get_translatable() {
		...
		if( strpos( $this->name, "/" ) === false ){
			...
			$this->wikimedia = true;
		} else if( strpos( $this->name, "http://" ) !== 0 ) {
			$url = 'http://'.$this->name;
		} else {
			$url = $this->name;
		} 
	...
	public function get_footer_text(){
		global $I18N;
		$html = "\t\t<p>";
		$html .= _html( 'stats-footer', array( 'variables' => array( $this->get_counter(), $I18N->dateFormatted( 'March 2011', 'F Y' ) ) ) );
	...
	private function generate_first_form() {
		...
		$html .= '<p>' . $I18N->msg( 'translate-instructions', array( 'variables' => array( '<em>' . _html( 'format-filename-example' ) . '</em>', '<em>' . _html( 'format-fullurl-example' ) . '</em>' ) ) ) . '</p>';
